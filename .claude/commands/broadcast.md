...
Load the `marketing` skill and execute the Broadcast — 3-Gate Review:
...
ssh root@<RUN_SERVER> << 'REMOTE'
cd <PROD_PATH>
echo "y" | uv run python scripts/broadcast.py --env prod --file docs/marketing/broadcast_{feature}.txt --max-level {N} --button "Label" callback
REMOTE
```
